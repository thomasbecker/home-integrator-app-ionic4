...
const ENVIRONMENT_WITH_HISTORY_URL = ENVIRONMENT_SERVER_URL + '/api/homeEnvironmentData/10?from=';
const ENVIRONMENT_LIVE_URL = ENVIRONMENT_SERVER_URL + '/api/homeEnvironmentData/live/10';
...
export class DataProviderWs {
    ...
    public getEnvironmentMessages(): Observable<HomeEnvironmentData> {
        return DataProviderWs.homeEnvironmentSubject ? DataProviderWs.homeEnvironmentSubject : this.getHomeEnvironmentDataObservable();
    }
    ...
    private getHomeEnvironmentDataObservable() {
        ...
        DataProviderWs.homeEnvironmentSubject = replaySubject
            .pipe(map((msg): HomeEnvironmentData => {
    ...
    public getEnvironmentMessagesWithHistory(timestampStartHistory): Observable<HomeEnvironmentData> {
        return DataProviderWs.homeEnvironmentWithHistorySubject ?
            DataProviderWs.homeEnvironmentWithHistorySubject : this.getHomeEnvironmentDataWithHistoryObservable(timestampStartHistory);
    }

    private getHomeEnvironmentDataWithHistoryObservable(timestampStartHistory): Observable<HomeEnvironmentData> {
        const replaySubject = new ReplaySubject(1000);
        const webSocketSubject = this.wsService
            .connect(ENVIRONMENT_WITH_HISTORY_URL + timestampStartHistory);
        webSocketSubject.subscribe(replaySubject);
        DataProviderWs.homeEnvironmentWithHistorySubject = replaySubject
            .pipe(map((msg): HomeEnvironmentData => {
                return DataProviderWs.mapEnvironment(msg);
            }));
        return DataProviderWs.homeEnvironmentWithHistorySubject;
    ...
    public getPowerMessages(): Observable<HomePowerData> {
        DataProviderWs.homePowerSubject = this.wsService
            ...
            .pipe(map((msg): HomePowerData => {
                return this.mapPower(msg);
            }));
        return DataProviderWs.homePowerSubject;
    }

    public getPowerMessagesWithHistory(timestampStartHistory): Observable<HomePowerData> {
        DataProviderWs.homePowerWithHistorySubject = <Subject<HomePowerData>>this.wsService
